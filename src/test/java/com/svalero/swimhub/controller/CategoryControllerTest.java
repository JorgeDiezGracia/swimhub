...
public class CategoryControllerTest {
    ...
    void findAll_returnsOk() throws Exception {
        ...
        when(categoryService.findAll()).thenReturn(List.of(c1));

        mockMvc.perform(get("/api/categories"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].name").value("Benjamín"));
    }
    ...
    void findById_existingId_returnsOk() throws Exception {
        ...
        when(categoryService.findById(1L)).thenReturn(c);

        mockMvc.perform(get("/api/categories/1"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.name").value("Benjamín"));
    }
